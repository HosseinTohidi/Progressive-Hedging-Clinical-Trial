...
ip=1 #of potential facilities (cluster of facilities)
j= 2 # of drugs to be tested
t=50  #planning horizon
r=1 # of resources
...
model.addConstrs((quicksum(f[i,j,t,omega] for i in I) >= N[j,omega] for (j,t) in Tj if t==T[-1] for omega in Omega),'c7_1')
...
model.addConstrs((m[i,j]+quicksum( mp[i,j,t,omega] for t in T)<=1 for i in I for j in J for omega in Omega),'c15')
